...
class SerialConfigWidget(QFrame):
    ...
    def __init__(self, title, parent_page=None, parent=None):
    ...
    def send_data(self):
        if not self.serial.isOpen():
            if self.parent_page:
                self.parent_page.show_error("Error", "Serial port is not open")
            return
        ...
        try:
            ...
            if self.parent_page:
                self.parent_page.show_error("Error", str(e))
...
class SerialDashboardPage(BasePage):
    ...
    def add_card(self):
        if len(self.cards) >= 4:
            ...
            return
        card = SerialConfigWidget(f"Serial {len(self.cards) + 1}", parent_page=self)
